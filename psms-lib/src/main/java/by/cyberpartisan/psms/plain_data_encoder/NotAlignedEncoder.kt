...
abstract class NotAlignedEncoder : PlainDataEncoder {
        ...
        for (c in s) {
            ...
            while (code.size > 0) {
                ...
                val valueForByte = if (code.size > bitsLeftInLastByte) {
                    ...
                    code.value shl (bitsLeftInLastByte - code.size)
                }
                ...
                if (code.size < bitsLeftInLastByte) {
                    ...
                    code.value = code.value and (255 shl code.size).inv()
    ...
    override fun decode(data: ByteArray): String {
        ...
        while (data.size * 8 - currentOffset >= decodingShifting) {
            ...
            if (byteIndex == data.size - 1 &&
                8 - (getLeastSignificantBitOffset(data[byteIndex]) + 1) < byteOffset + decodingShifting
            ) break
            var value = data[byteIndex].toInt() and (255 shr byteOffset)
