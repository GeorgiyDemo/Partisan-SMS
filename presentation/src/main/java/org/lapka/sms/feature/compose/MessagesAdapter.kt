...
import org.lapka.sms.common.util.TextViewStyler
...
import org.lapka.sms.common.util.extensions.setPadding
...
import org.lapka.sms.feature.compose.BubbleUtils.canGroup
import org.lapka.sms.feature.compose.BubbleUtils.getBubble
...
import org.lapka.sms.common.widget.TightTextView
...
class MessagesAdapter @Inject constructor(
    ...
    override fun onBindViewHolder(holder: QkViewHolder, position: Int) {
        ...
        holder.itemView.findViewById<QkTextView>(R.id.timestamp).setVisible(
            timeSincePrevious >= BubbleUtils.TIMESTAMP_THRESHOLD
                    || message.subId != previous?.subId && subscription != null
        )

        holder.itemView.findViewById<ImageView>(R.id.sim)
            .setVisible(message.subId != previous?.subId && subscription != null && subs.size > 1)
        holder.itemView.findViewById<QkTextView>(R.id.simIndex)
            .setVisible(message.subId != previous?.subId && subscription != null && subs.size > 1)
        ...
        holder.itemView.findViewById<TightTextView>(R.id.body).setBackgroundResource(
            getBubble(
                emojiOnly = emojiOnly,
    ...
    private fun bindStatus(holder: QkViewHolder, message: Message, next: Message?) {
        ...
        holder.itemView.findViewById<QkTextView>(R.id.status).setVisible(
            when {
